...
import bibtexparser
import glob
#accepts the queryes and makes the summary


def couple_bib_files(query):

    #query = str(query)
    os.chdir(f"downloads/{query}")
    # Задайте путь к директории с файлами BibTeX
    bibtex_dir = os.getcwd()
    ...
    path_t = os.getcwd()
    with open(bibtex_dir + f"/combined {query}.bib", 'w', encoding="utf-8") as combined_bibtex_file:
